...
function acf_flush_field_group_cache( $field_group ) {
	...
	wp_cache_delete( acf_cache_key( "acf_get_field_group_post:key:{$field_group['key']}" ), 'osen' );
	
	// Flush cached array of post_ids for collection of field groups.
	wp_cache_delete( acf_cache_key( "acf_get_field_group_posts" ), 'osen' );
}
...
function acf_duplicate_field_group( $id = 0, $new_post_id = 0 ){
	...
	if( !$new_post_id ) {
		$field_group['title'] .= ' (' . __("copy", 'osen') . ')';
	}
