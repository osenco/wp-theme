...
<p><?php _e( 'Choose the type of content where you want to use the custom template, then click on the save or update button (for Gutenberg you must also refresh the page to show avaiable custom fields).', 'osen' ); ?></p>
<?php } ?>
<div class="wp-field-row">
    <label for="used_by" title="<?php _e( 'Used by', 'osen' ); ?>"><?php _e( 'Used by', 'osen' ); ?></label>
    <select id="used_by" name="template_settings[field_used_by]" onchange="set_cpt_fields('used_by');">
        <option value=""><?php _e( '- Choose -', 'osen' ); ?></option>
        <optgroup label="<?php _e( 'Post types', 'osen' ); ?>">
            <option value="post"<?php selected( $template_used_by, 'post' ); ?>><?php _e( 'Posts', 'osen' ); ?></option>
            <option value="page"<?php selected( $template_used_by, 'page' ); ?>><?php _e( 'Pages', 'osen' ); ?></option>
            <?php
